...
def multiplicative_attention(dec, queries, keys, values, bias, head_dim, positions, window_size, gernal_heads, local_heads, long_heads, keep_prob=None,
                             name=None):
	...
	################################################
        if (local_heads) or (long_heads):	
            ...
            mask_lens = tf.reduce_sum(tf.exp(bias),-1,keep_dims=True) - 1.0
            molecular_p = tf.reshape(tf.to_float(tf.range(key_len)),[1,1,1,key_len])
			
	    #for general heads
            if gernal_heads:
	        gaussian.append(tf.ones([batch_size,gernal_heads,query_len,key_len]))			
			
            #for local attention
            if local_heads: 	
	        molecular = tf.tile(positions * mask_lens, [1,1,1,key_len]) - tf.tile(molecular_p, [batch_size,local_heads,query_len,1])
                if dec:
                    k_s = combine_heads(tf.matmul(tf.tile(k_mask,[batch_size,num_heads,1,1]),keys)/(mask_lens + 1.0))
                else:
                    k_s = combine_heads(tf.matmul(tf.tile(k_mask,[1,num_heads,1,1]),keys)/(mask_lens + 1.0))
            ...
            #weights = tf.multiply(weights, tf.concat(gaussian,axis=1))
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        if local_heads:
            ...
            #k_len = k_shape[1]
            #k_s = tf.reduce_sum(k*tf.reshape(k_mask,[batch_size,k_len,1]),axis=1,keep_dims=True)/tf.reduce_sum(k_mask,axis=-1,keep_dims=False)
            #local_size = tf.nn.tanh(linear(k_s, local_heads*key_size//num_heads, True, True, scope="k_window_transform1"))
            #local_size = split_heads(tf.nn.sigmoid(linear(local_size, local_heads, True, True, scope="k_window_transform2")), local_heads)

            positions = tf.nn.tanh(linear(q, local_heads*key_size//num_heads, True, True, scope="q_pos_transform1"))
        ...
        # attention
        results = multiplicative_attention(dec, q, k, v, bias, key_depth_per_head, positions, local_size, num_heads-local_heads-long_heads, local_heads, long_heads, keep_prob)
