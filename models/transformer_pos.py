            ...
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    if (layer == 0)or(layer==1)or(layer == 2):
                        ...
                        localheads = 0
                    y = layers.attention_pos_dyn.multihead_attention(
    ...
    with tf.variable_scope(scope, default_name="decoder", dtype=dtype,
        ...
        for layer in range(params.num_decoder_layers):
            ...
            with tf.variable_scope(layer_name):
                ...
                with tf.variable_scope("self_attention"):
                    y = layers.attention_pos.multihead_attention(
                        _layer_process(x, params.layer_preprocess),
                        None,
                        bias,
                        params.num_heads,
...
def encoding_graph(features, mode, params):
    ...
    encoder_input = layers.attention_pos.add_timing_signal(encoder_input)
    enc_attn_bias = layers.attention_pos.attention_bias(src_mask, "masking")
...
def decoding_graph(features, state, mode, params):
    ...
    enc_attn_bias = layers.attention_pos.attention_bias(src_mask, "masking")
    dec_attn_bias = layers.attention_pos.attention_bias(tf.shape(targets)[1],
                                                    "causal")
    # Shift left
    decoder_input = tf.pad(targets, [[0, 0], [1, 0], [0, 0]])[:, :-1, :]
    decoder_input = layers.attention_pos.add_timing_signal(decoder_input)
