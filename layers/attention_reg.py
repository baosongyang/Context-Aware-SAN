...
def multiplicative_attention(queries, keys, values, bias, positions, window_size, gernal_heads, local_heads, long_heads, keep_prob=None,
                             name=None):
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        # attention
        results = multiplicative_attention(q, k, v, bias, positions, local_size, num_heads-local_heads-long_heads, local_heads, long_heads, keep_prob)
        ...
        reg_loss = 0.0
        if local_heads:
            q_mask = tf.transpose(tf.exp(bias),[0,1,3,2])
            cul_sub = 0
            for i in range(local_heads-1):
                cul_sub += local_heads-1-i
                reg_loss += tf.reduce_sum(tf.pow(1.0-tf.abs(positions[:,i:i+1,:,:]-positions[:,i+1:,:,:])*q_mask,2.0))
            reg_loss = reg_loss/(tf.reduce_sum(q_mask)*cul_sub)

        return outputs,0.1*reg_loss		
