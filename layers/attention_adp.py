...
def multiplicative_attention(queries, keys, values, bias, positions, keep_prob=None,
                             name=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        if bias is not None:
            logits += bias
        weights = tf.nn.softmax(logits*positions,name="attention_weights")
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        #positions=None
        #if local_heads:
        positions = tf.exp(split_heads(tf.nn.tanh(linear(q, num_heads, True, True, scope="q_pos_transform1")),num_heads))
        ...
        # attention
        results = multiplicative_attention(q, k, v, bias, positions, keep_prob)
