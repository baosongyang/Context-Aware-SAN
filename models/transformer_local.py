        ...
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    #if (layer == 3)or(layer == 4)or(layer==5):localheads = params.local_heads
                    #else: localheads=0
                    #if (layer == 3)or(layer == 4):longheads = params.long_heads
                    #else: longheads=0
                    if (layer == 1)or(layer==2)or(layer == 3):
                        local_model = layers.attention_dyn
                        localheads = params.local_heads
                    elif (layer == 4)or(layer ==5):
                        local_model = layers.attention_avg
                        localheads = params.local_heads
                    else:
                        local_model = layers.attention_dyn
                        localheads = 0
                    ...
                    y = local_model.multihead_attention(
    ...
    with tf.variable_scope(scope, default_name="decoder", dtype=dtype,
        ...
        for layer in range(params.num_decoder_layers):
            ...
            with tf.variable_scope(layer_name):
                #if (layer == 0)or(layer == 1):localheads = params.local_heads
                #else: localheads = 0
                if (layer == 1)or(layer==2)or(layer == 3):
                    local_model = layers.attention_dyn
                    localheads = params.local_heads
                elif (layer == 4)or(layer ==5):
                    local_model = layers.attention_avg_dec
                    localheads = params.local_heads
                else:
                    local_model = layers.attention_dyn
                    localheads = 0
