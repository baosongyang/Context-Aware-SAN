...
def multihead_attention(queries, memories, bias, prectx,src_q,src_k, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        #key_depth_per_head = key_size
        #q_ctx = linear(curctx, key_size, True, scope="q_context_transform")
        #q_ctx *= key_depth_per_head ** -0.5

        #src_q,src_k = tf.split(linear(prectx, 2*key_size, True, scope="k_context_transform"),[key_size,key_size],axis=-1)

######################################################################################
        global_q,global_k = tf.split(linear(prectx,2*key_size,False,True,scope="global_context_transform"),[key_size,key_size],axis=-1)
        
        #q_ctx = split_heads(q_ctx, 2)
        #k_ctx = split_heads(k_ctx, 2)

        #ctx_logit = tf.matmul(q_ctx,k_ctx,transpose_b=True)

        #src_weights = tf.nn.softmax(ctx_logit,name="ctx_weights")[:,:,0,:]
       
        #src_q,src_k = tf.split(tf.matmul(src_weights,v_ctx),[1,1], axis=1)

        #src_q = linear(src_q,key_size,False,True,scope="src_q_transform")
        #src_context = linear(tf.matmul(src_weights,v_ctx), 2*key_size, True, scope="source_context_transform")
        #src_k = linear(src_k,key_size,False,True,scope="src_k_transform")

        #src_q,src_k = tf.split(src_context, [key_size, key_size], axis=-1)
        ###########################################################################
        global_q = split_heads(global_q,num_heads)
        global_k = split_heads(global_k,num_heads)
        
        src_q = split_heads(src_q,num_heads)
        src_k = split_heads(src_k,num_heads)
        ...
        #context_bias = tf.tile(tf.exp(bias),[1,num_heads,1,1])
        # split heads
        ...
        #context = tf.reshape(tf.transpose(tf.matmul(context_mask,v)/mask_len,[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        #context_q = tf.reshape(tf.transpose(tf.matmul(context_bias,q)/mask_len,[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        ...
        #context_q = tf.transpose(tf.reshape(tf.matmul(context_q,context_matrix_q),[num_heads,-1,k_len,key_depth_per_head]),[1,0,2,3])
        
        #context_k = tf.reshape(tf.transpose(tf.matmul(context_bias,k)/mask_len,[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        #context_matrix_k = tf.get_variable("context_matrix_k", [num_heads,key_depth_per_head, key_depth_per_head], dtype=dtype)
        #context_k = tf.transpose(tf.reshape(tf.matmul(context_k,context_matrix_k),[num_heads,-1,k_len,key_depth_per_head]),[1,0,2,3])
        q = (q + src_q + global_q)/3.0
       #q += 0.1*src_q
        k = (k + src_k + global_k)/3.0
        #k += 0.1*src_k
