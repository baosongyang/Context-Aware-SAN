...
def transformer_encoder(inputs, bias, params, dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="encoder", dtype=dtype,
        ...
        src_bias = tf.exp(bias[:,0,:,:])
        src_len = tf.reduce_sum(src_bias,-1,keep_dims=True)
        src_context= tf.zeros([tf.shape(x)[0], 1, 0])
        ...
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    ...
                    src_context = tf.concat([src_context,tf.matmul(src_bias,x)/src_len],axis=-1)
        ...
        return outputs,src_context


def transformer_decoder(inputs, memory,src_context, bias, mem_bias, params, state=None,
                        dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="decoder", dtype=dtype,
                           values=[inputs, memory, bias, mem_bias]):
        x = inputs
        next_state = {}
        #k_len = tf.shape(mem_bias)[3]
        #src_bias = tf.exp(mem_bias[:,0,:,:])
        #src_len = tf.reduce_sum(src_bias,-1,keep_dims=True)
        #src_context = tf.matmul(src_bias,memory)/src_len
...
def encoding_graph(features, mode, params):
    ...
    encoder_output,src_context = transformer_encoder(encoder_input, enc_attn_bias, params)

    return encoder_output,src_context
...
def decoding_graph(features, state, mode, params):
    ...
    src_context = state["context"]

    if mode != "infer":
        decoder_output = transformer_decoder(decoder_input, encoder_output, src_context,
                                             dec_attn_bias, enc_attn_bias,
                                             params)
    else:
        decoder_input = decoder_input[:, -1:, :]
        dec_attn_bias = dec_attn_bias[:, :, -1:, :]
        decoder_outputs = transformer_decoder(decoder_input, encoder_output, src_context,
                                              dec_attn_bias, enc_attn_bias,
        ...
        return log_prob, {"encoder": encoder_output,"context": src_context,  "decoder": decoder_state}
...
def model_graph(features, mode, params):
    encoder_output,src_context = encoding_graph(features, mode, params)
    state = {
...
class Transformer(interface.NMTModel):
    ...
    def get_inference_func(self):
        def encoding_fn(features, params=None):
            ...
            with tf.variable_scope(self._scope):
                encoder_output,src_context = encoding_graph(features, "infer", params)
