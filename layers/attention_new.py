...
def multiplicative_attention(queries, keys, values, k_pos,  bias, positions, window_size, gernal_heads, local_heads, keep_prob=None,
                             name=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        if k_pos is not None:
            num_heads = shape[1]
            queries = tf.reshape(tf.transpose(queries,[2,0,1,3]),[query_len,batch_size*num_heads,-1])
            logits +=  tf.reshape(tf.transpose(tf.matmul(queries, k_pos, transpose_b=True),[1,0,2]),shape)
	...
	#localness
	    ...
	    gaussian.append(0.0 - 2*tf.pow(tf.div(molecular,window_size),2.0))
...
def multihead_attention(queries, memories, bias,  num_heads, key_size,
                        ...
                        state=None, dtype=None, scope=None, prectx=None, k_size=0, local_heads=0):
    ...
    if local_heads > num_heads:
        raise ValueError("totoal heads number (%d) larger than num_heads (%d) must be divisible by the number of " % (local_heads, num_heads))	 
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        #context
        if prectx is not None:
	    len_q = tf.shape(q)[-2]
            global_q,global_k = tf.split(linear(prectx,2*key_size,False,True,scope="global_context_transform"),[key_size,key_size],axis=-1)
            q_gate = tf.nn.sigmoid(linear(tf.concat([q,tf.tile(global_q,[1,len_q,1])],axis=-1),1,True,True,scope="gate_q"))
            k_gate = tf.nn.sigmoid(linear(tf.concat([k,tf.tile(global_k,[1,len_q,1])],axis=-1),1,True,True,scope="gate_k"))       
            q = (1-q_gate)*q + q_gate*(global_q)
            k = (1-k_gate)*k + k_gate*(global_k)
        
        key_depth_per_head = key_size // num_heads
        #relative position
        k_pos = None
        if k_size:
            pos_rep_k = tf.get_variable("pos_k_emb", [2*k_size+1, key_depth_per_head], dtype=dtype)
            len_k = tf.shape(k)[-2]
            shape_q = tf.shape(q)
            len_q = shape_q[-2]
            if state is not None:
                indices = tf.maximum(tf.minimum(tf.range(k_size,k_size+len_k)-(len_k-1), 2*k_size),0)
            else:
                indices = tf.maximum(tf.minimum(tf.tile(tf.range(k_size,k_size+len_k),[len_q]) - tf.reshape(tf.tile(tf.reshape(tf.range(len_q),[len_q,1]),[1,len_k]), [len_q*len_k]), 2*k_size),0)
            k_pos = tf.reshape(tf.gather(pos_rep_k,indices),[len_q,len_k,key_depth_per_head])
        #localness
        positions=None
        local_size =None
        if local_heads:
            pos_window = tf.nn.tanh(linear(q, local_heads*key_size//num_heads, True, True, scope="q_pos_transform1"))
            pos_window_split = split_heads(tf.nn.sigmoid(linear(pos_window, 2*local_heads, True, True, scope="q_pos_transform2")), 2*local_heads)
            positions = pos_window_split[:,:local_heads,:,:]
            local_size = pos_window_split[:,local_heads:,:,:]
        # split heads
        ...
        # attention
        results = multiplicative_attention(q, k, v, k_pos, bias, positions, local_size, num_heads-local_heads, local_heads, keep_prob)
