...
def multihead_attention(queries, memories, bias, num_heads, key_size,
                        ...
                        state=None, dtype=None, scope=None, z_size = 128):
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        if memories is None:
            ...
            if state is not None:
                ...
                #qz=linear(queries,z_size,False,True,scope="qz_transform")
                k_mask = tf.reshape(tf.exp(bias),[batch_size,1,k_length])
                mask_lens = tf.reduce_sum(k_mask,-1,keep_dims=True)
                mz= linear(tf.matmul(k_mask, queries)/mask_lens,z_size,True,True,scope="mz_transform")
                #z= tf.nn.tanh(qz+mz)
                z= tf.nn.tanh(mz)

                dynamic_W = linear(tf.matrix_diag(tf.reshape(z,[batch_size,z_size])),key_size,False,True,scope="P_transform")
                dynamic_W = linear(tf.transpose(dynamic_W,[0,2,1]),size,False,True,scope="Q_transform")
                dynamic_b = linear(z,size,False,True,scope="b_transform")

                combined = tf.reshape(tf.matmul(queries,dynamic_W),[batch_size,k_length,size])+dynamic_b
                q, k, v = tf.split(combined, [key_size, key_size, value_size],axis=-1)
            ...
            k_mask = tf.reshape(tf.exp(bias),[batch_size,1,k_length])
            mask_lens = tf.reduce_sum(k_mask,-1,keep_dims=True)
            mz= linear(tf.matmul(k_mask, memories)/mask_lens,z_size,True,True,scope="mz_transform")
            #z= tf.nn.tanh(qz+mz)
            z= tf.nn.tanh(mz)
            size = key_size  + value_size
            dynamic_W = linear(tf.matrix_diag(tf.reshape(z,[batch_size,z_size])),key_size,False,True,scope="P_transform")
            dynamic_W = linear(tf.transpose(dynamic_W,[0,2,1]),key_size+size,False,True,scope="Q_transform")
            dynamic_W_c,dynamic_W_m = tf.split(dynamic_W,[key_size,size],axis=-1)
            dynamic_b = linear(z,key_size+size,False,True,scope="b_transform")
            dynamic_b_c,dynamic_b_m = tf.split(dynamic_b,[key_size,size],axis=-1)
            q = tf.reshape(tf.matmul(queries,dynamic_W_c),[batch_size,k_length,key_size])+dynamic_b_c
            combined = tf.reshape(tf.matmul(memories,dynamic_W_m),[batch_size,k_length,size])+dynamic_b_m
