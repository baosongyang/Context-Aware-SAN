...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        #src_k = linear(memories, key_size, True, scope="source_context_transform")
        #src_q,src_k = tf.split(src_context, [key_size, key_size], axis=-1)

        src_q,src_k = tf.split(linear(memories, 2*key_size, False, True, scope="context_transform"),[key_size,key_size],axis=-1)
