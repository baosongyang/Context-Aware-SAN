    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        if memories is None:
            ...
            if state is not None:
                qs = tf.concat([state["query1"], q], axis=1)
                k = tf.concat([state["key"], k], axis=1)
                v = tf.concat([state["value"], v], axis=1)
                next_state["query1"] = qs
                next_state["query2"] = state["query2"]
                next_state["key"] = k
                next_state["value"] = v
        else:
            q = linear(queries, key_size, True, True, scope="q_transform")
            if state is not None:
                qs = tf.concat([state["query2"], q], axis=1)
                next_state["query1"] = state["query1"]
                next_state["query2"] = qs
        ...
        def tf_while_body(x,loop_counter):
            loop_counter -= 1
            y = tf.concat(([x[0]],x[:-1]),axis=2)
            z = tf.reduce_maximum(x,y)
            return z,loop_counter

        if state is None:
            #q_len = tf.shape(q)[-2]
            #lower_triangle = tf.matrix_band_part(tf.ones([q_len, q_len]),-1, 0)
            #ret = (-1e9) * (1.0 - lower_triangle)
            #forward=tf.reshape(ret, [1, 1, q_len, q_len])
            #context_mask = tf.tile(tf.exp(forward),[batch_size,num_heads,1,1])
            #mask_len = tf.reduce_sum(context_mask,-1,keep_dims=True)
            max_q = tf.transpose(q,[2,0,1,3])
            context_q= tf.scan(lambda a, y: tf.reduce_max(tf.concat([[a], [y]],axis=0),0), max_q, initializer=None, parallel_iterations=1,back_prop=True, swap_memory=False)
            context_q = tf.reshape(tf.transpose(context_q,[2,1,0,3]),[num_heads,-1,key_depth_per_head])
            #context_q = tf.reshape(tf.transpose(tf.matmul(context_mask,q)/mask_len,[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        else:
            qs = split_heads(qs, num_heads)
            #context_q = tf.reshape(tf.transpose(tf.reduce_mean(qs,-2,keep_dims=True),[1,0,2,3]),[num_heads,-1,key_depth_per_head])
            context_q = tf.reshape(tf.transpose(tf.reduce_max(qs,-2,keep_dims=True),[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        ...
        if memories is None:
            ...
            #    context_mask = tf.tile(tf.exp(forward),[batch_size,num_heads,1,1])
            #    mask_len = tf.reduce_sum(context_mask,-1,keep_dims=True)
            max_k = tf.transpose(k,[2,0,1,3])
            context_k= tf.scan(lambda a, y: tf.reduce_max(tf.concat([[a], [y]],axis=0),0), max_k, initializer=None, parallel_iterations=1,back_prop=True, swap_memory=False)
            context_k = tf.reshape(tf.transpose(context_k,[2,1,0,3]),[num_heads,-1,key_depth_per_head])
            #context_k = tf.reshape(tf.transpose(tf.matmul(context_mask,k)/mask_len,[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        else:
