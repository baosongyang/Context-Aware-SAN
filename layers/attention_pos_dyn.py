...
def multiplicative_attention(queries, keys, values, k_pos, v_pos, bias,positions, window_size, local_heads, keep_prob=None, name=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        if local_heads:
            ...
            logits += 0.0 - 2*tf.pow(tf.div(molecular,window_size),2.0)
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
                        ...
                        state=None, dtype=None, scope=None, k_size = 0, local_size = 10, local_heads=0):
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        positions = None
        if k_size:
            pos_rep_kv = tf.get_variable("pos_kv_emb", [2*k_size+1, (key_size+value_size)//num_heads], dtype=dtype)
        ...
        # attention
        results = multiplicative_attention(q, k, v, k_pos, v_pos, bias, positions, local_size, local_heads, keep_prob)
