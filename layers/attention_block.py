...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        q_len = q_shape[2]
        k_len = tf.shape(k)[2]
        bias = tf.reshape(bias,[batch_size,1,k_len//4,1,4])
        ...
        q=tf.reshape(q,[batch_size,num_heads,m_len,4,key_depth_per_head])
        k=tf.reshape(k,[batch_size,num_heads,m_len,4,key_depth_per_head])
        v=tf.reshape(v,[batch_size,num_heads,m_len,4,key_depth_per_head])

        # attention
        results = multiplicative_attention(q, k, v, bias, keep_prob)
        result1 = tf.reshape(results["outputs"],[batch_size,num_heads,q_len,key_depth_per_head])
        h = tf.nn.sigmoid(linear(combine_heads(result1),key_size,True,True,scope="q2_transform"))

        logits2 = tf.reshape(split_heads(linear(h,num_heads,True,True,scope="logit2_transform"),num_heads),[batch_size,num_heads,m_len,1,4])
        weights2 = tf.nn.softmax(logits2,name="s2t_weights")
        v_=tf.reshape(tf.matmul(weights2,results["outputs"]),[batch_size,num_heads,m_len,key_depth_per_head])

        v_q = split_heads(linear(combine_heads(v_),key_size,True,True,scope="v_q_transform"),num_heads)
        v_q *= key_depth_per_head ** -0.5

        o = combine_heads(multiplicative_attention(v_q,v_,v_,None,keep_prob)["outputs"])
        v_ = combine_heads(v_)
        
        G = tf.nn.sigmoid(linear(tf.concat([v_,o],axis=-1),key_size,True,True,scope="G_transform"))
        e = split_heads(G*o + (1.0-G)*v_,num_heads)

        e = tf.reshape(e,[batch_size,num_heads,m_len,1,key_depth_per_head])
        e = tf.reshape(tf.tile(e,[1,1,1,4,1]),[batch_size,num_heads,q_len,key_depth_per_head])
        x = tf.concat([result1,e],axis=-1)
