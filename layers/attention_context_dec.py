...
def multihead_attention(queries, memories,global_q,global_k, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        q_gate = tf.nn.sigmoid(linear(tf.concat([q,global_q],axis=-1),1,True,True,scope="gate_q"))
        k_gate = tf.nn.sigmoid(linear(tf.concat([k,global_k],axis=-1),1,True,True,scope="gate_k"))
