	...
	################################################
	    ...
	    #for general heads
            ...
            if local_heads:
                ...
                pos_window = split_heads(tf.nn.sigmoid(linear(pos_window, 2*local_heads, True, True, scope="pos_transform")),2*local_heads)
	        ...
	        molecular = tf.tile(positions * mask_lens, [1,1,1,key_len]) - tf.tile(molecular_p, [batch_size,local_heads,query_len,1])
                window_size = tf.tile((window_size * mask_lens)+0.1,[1,1,1,key_len])
