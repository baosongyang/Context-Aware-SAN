...
def multiplicative_attention(queries, keys, values, bias, head_dim, window_size, gernal_heads, local_heads, long_heads, keep_prob=None,
                             name=None):
	...
	################################################
	    ...
	    #for general heads
            ...
            if local_heads:
                ...
                v_pos = linear(first_values, head_dim, False, True, scope="v_pos_transform")
                positions = tf.nn.tanh(p_pos+v_pos)
                positions = tf.nn.sigmoid(linear(positions, 1, True, True, scope="pos_transform"))
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        # split heads
        q = layer_norm(split_heads(q, num_heads))
        k = layer_norm(split_heads(k, num_heads))
        v = layer_norm(split_heads(v, num_heads))
        ...
        # attention
        results = multiplicative_attention(q, k, v, bias, key_depth_per_head, local_size, num_heads-local_heads-long_heads, local_heads, long_heads, keep_prob)
