...
def multiplicative_attention(queries, keys, values, bias,num_heads,reverse_di, keep_prob=None,
                             name=None):
	...
	if reverse_di:
        	gaussian=tf.tile(tf.concat([forward,backward],axis=1),[1,int(num_heads/2),1,1])
	else:
        	gaussian=tf.tile(tf.concat([backward,forward],axis=1),[1,int(num_heads/2),1,1])
	logits += gaussian
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        # attention
        context = multiplicative_attention(q, k, v, bias, num_heads, False, keep_prob)["outputs"]
