...
def transformer_decoder(inputs, memory, bias, mem_bias, params, state=None,
    ...
    with tf.variable_scope(scope, default_name="decoder", dtype=dtype,
        ...
        if state is not None:
            global_context = tf.zeros([batch_size,1,0])
            deep_context = tf.zeros([batch_size,1,0])
        else:
            k_len = tf.shape(x)[1]
            trg_bias = tf.tile(tf.exp(bias[:,0,:,:]),[batch_size,1,1])
            trg_len = tf.maximum(1.0, tf.reduce_sum(trg_bias,-1,keep_dims=True))
            global_context = tf.zeros([batch_size,k_len,0])
            deep_context = tf.zeros([batch_size,k_len,0])
        for layer in range(params.num_decoder_layers):
            ...
            with tf.variable_scope(layer_name):
                ...
                with tf.variable_scope("self_attention"):
                    x = _layer_process(x, params.layer_preprocess)
                    if state is not None:
                        prectx = tf.concat([layer_state["query1"],x],axis=1)
                        #next_state["query1"]=prectx
                        global_context = tf.concat([global_context,tf.reduce_mean(prectx,1,keep_dims=True)],axis =-1)
                    else:
                        global_context = tf.concat([global_context,tf.matmul(trg_bias,x)/trg_len],axis =-1)
                    
                    deep_context = tf.concat([deep_context,x],axis=-1)
                    
                    global_q,global_k = tf.split(layers.nn.linear(global_context,2*params.hidden_size,False,True,scope= "global_context_transform"),[params.hidden_size,params.hidden_size],axis=-1)        
                    if layer == 0:
                        deep_q = tf.zeros(tf.shape(global_q))
                        deep_k = tf.zeros(tf.shape(global_k))
                    else:
                        deep_q,deep_k = tf.split(layers.nn.linear(deep_context[:,:,:-params.hidden_size],2*params.hidden_size,False,True,scope="deep_transform"),[params.hidden_size,params.hidden_size],axis=-1)
                    global_q += deep_q
                    global_k += deep_k
                    if state is not None:
                        global_k = tf.concat([layer_state["query2"],global_k],axis=1)
                    y = layers.attention_context_dec.multihead_attention(
                        #_layer_process(x, params.layer_preprocess),
                        x,
                        None,
                        global_q,
                    ...
                    if layer_state is not None:
                        ...
                        next_state[layer_name]["query1"]=prectx
                        next_state[layer_name]["query2"]=global_k
        ...
        def encoding_fn(features, params=None):
            ...
            with tf.variable_scope(self._scope):
                ...
                state = {
                    ...
                    "decoder": {
                        "layer_%d" % i: {
                            "query1": tf.zeros([batch, 0, params.hidden_size]),
                            "query2": tf.zeros([batch, 0, params.hidden_size]),
