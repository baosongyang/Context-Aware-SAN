...
def multiplicative_attention(queries, keys, values, bias, p_pos, head_dim,  window_size, gernal_heads, local_heads, long_heads, keep_prob=None,
                             name=None):
    ...
    with tf.name_scope(name, default_name="multiplicative_attention",
        ...
        weights = tf.nn.softmax(logits,name="attention_weights")
	################################################
        if (local_heads) or (long_heads):	
            #positions shape [batch, heads, length_q, 1]
            #gaussian
            ...
            key_len = shape[3]
            k_mask = tf.exp(bias)
            mask_lens = tf.reduce_sum(k_mask,-1,keep_dims=True) - 1.0
            molecular_p = tf.reshape(tf.to_float(tf.range(key_len)),[1,1,1,key_len])
			
	    #for general heads
            if gernal_heads:
	        gaussian.append(tf.zeros([batch_size,gernal_heads,query_len,key_len]))			
			
            #for local attention
            if local_heads:
                #p_pos = linear(queries[:,gernal_heads:gernal_heads+local_heads,:,:], head_dim, True, True, scope="q_pos_transform")
                first_values = tf.matmul(weights[:,gernal_heads:gernal_heads+local_heads,:,:], values[:,gernal_heads:gernal_heads+local_heads,:,:])
                v_pos = linear(combine_heads(first_values), head_dim*local_heads, False, True, scope="v_pos_transform")
                pos_window = tf.nn.tanh(p_pos+v_pos)
                positions = split_heads(tf.nn.sigmoid(linear(pos_window, local_heads, True, True, scope="pos_transform")),local_heads)
                #positions = pos_window[:,:local_heads,:,:]
                #window_size = pos_window[:,local_heads:,:,:]
                k_s = combine_heads(tf.matmul(tf.tile(k_mask,[1,num_heads,1,1]),keys))
                window_size  = tf.nn.tanh(linear(k_s, local_heads*head_dim, True, True, scope="k_window_transform1"))
                window_size = split_heads(tf.nn.sigmoid(linear(window_size, local_heads, True, True, scope="k_window_transform2")), local_heads)

	        molecular = tf.tile(positions * mask_lens, [1,1,1,key_len]) - tf.tile(molecular_p, [batch_size,local_heads,query_len,1])
                window_size = tf.tile((window_size * (mask_lens+1.0))+0.1,[1,1,1,key_len])
	        gaussian.append(0.0 - 2.0*tf.pow(tf.div(molecular,window_size),2.0))
				
            #for long distance attention 	
            if long_heads:
	        positions = tf.reshape(tf.to_float(tf.range(query_len)),[1,1,query_len,1])
	        molecular = tf.tile(positions, [batch_size,long_heads,1,key_len]) - tf.tile(molecular_p, [batch_size,long_heads,query_len,1])		 		
	        gaussian.append(1.0 - tf.exp(0.0 - 2.0*tf.pow(tf.div(molecular,window_size),2.0)))
            logits += tf.concat(gaussian,axis=1)
            #weights = tf.div(weights,tf.reduce_sum(weights,-1,keep_dims=True))
            
        weights = tf.nn.softmax(logits,name="attention_weights2")
...
def multihead_attention(queries, memories, bias, num_heads, key_size,
                        ...
                        state=None, dtype=None, scope=None, local_size = 10, local_heads=0, long_heads = 0):
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        positions=None
        if local_heads:
            positions = linear(q, local_heads*key_size//num_heads, True, True, scope="q_pos_transform1")
        #    positions = split_heads(tf.nn.sigmoid(linear(positions, local_heads, True, True, scope="q_pos_transform2")), local_heads)
        # split heads
        ...
        # attention
        results = multiplicative_attention(q, k, v, bias, positions, key_depth_per_head, local_size, num_heads-local_heads-long_heads, local_heads, long_heads, keep_prob)
