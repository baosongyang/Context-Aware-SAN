...
def transformer_encoder(inputs, bias, params, dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="encoder", dtype=dtype,
        ...
        #src_context= tf.zeros([tf.shape(x)[0], 0, 512])
        
        global_context = tf.zeros([tf.shape(x)[0],1,0])
        deep_context = tf.zeros([tf.shape(x)[0], k_len, 0])

        src_q = tf.zeros([tf.shape(x)[0], k_len, 512])
        src_k = tf.zeros([tf.shape(x)[0], k_len, 512])

        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    x = _layer_process(x, params.layer_preprocess)
                    global_context = tf.concat([global_context,tf.matmul(src_bias,x)/src_len],axis=-1)

                    y = layers.attention_context_di.multihead_attention(
                    ...
                    deep_context = tf.concat([deep_context,y],axis=-1)
                    if layer!=params.num_encoder_layers-1:
                        src_q,src_k = tf.split(layers.nn.linear(deep_context,2*params.hidden_size,False,True,scope="deep_context_transform"),[params.hidden_size,params.hidden_size],axis=-1)
                ...
                #new_context = tf.matmul(src_bias,x)/src_len
                #src_context = tf.concat([src_context,new_context],axis=1)
