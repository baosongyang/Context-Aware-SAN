...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        if memories is None:
            # self attention
            size = key_size * 3 + value_size
            combined = linear(queries, size, True, True, scope="qkv_transform")
            c, q, k, v = tf.split(combined, [key_size,key_size, key_size, value_size],
                               axis=-1)
        ...
        mask_lens = tf.reduce_sum(c_mask,-1,keep_dims=True)
        c = tf.matmul(tf.tile(tf.exp(bias),[1,num_heads,1,1]),split_heads(c,num_heads))/mask_lens
        q = split_heads(q, num_heads)+c
