...
def additive_attention(queries, memories, bias, num_heads, key_size,
                        value_size, output_size, keep_prob=None, output=True,
                        state=None, dtype=None, scope=None):
    """ Multi-head additive attention with input/output
        transformations.
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        ###################additive attention #####################
        q = tf.expand_dims(q,3)
        k = tf.expand_dims(k,2)
        logits = linear(tf.tanh(q+k),1,True,True,scope="logits")[:,:,:,:,0]
        #############################################################
