...
def multihead_attention(queries, memories, bias, num_heads, key_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        k_len = tf.shape(bias)[3]
        batch_size = tf.shape(k)[0]
        #mask_len = tf.reduce_sum(tf.exp(bias),-1,keep_dims=True)
        ...
        #context_mask = tf.tile(tf.exp(tf.concat([forward,backward],axis=1)+bias),[1,int(num_heads/2),1,1])
        #context_bias = tf.tile(tf.exp(bias),[1,num_heads,k_len,1])
        #context_bias = tf.transpose(tf.exp(bias),[0,1,3,2])

        #q_window = tf.nn.tanh(linear(q, key_size, True, True, scope="p_pos_transform1"))
        #q_window = split_heads(tf.nn.sigmoid(linear(q_window, num_heads, True, True, scope="q_pos_transform2")), num_heads)
        #k_window = tf.nn.tanh(linear(k, key_size, True, True, scope="k_pos_transform1"))
        #k_window = split_heads(tf.nn.sigmoid(linear(k_window, num_heads, True, True, scope="k_pos_transform2")), num_heads)
        #mask_lens = tf.reduce_sum(tf.exp(bias),-1,keep_dims=True)
        #q_window = tf.tile((q_window * (mask_lens+1.0))+0.1,[1,1,1,k_len])
        #k_window = tf.tile((k_window * (mask_lens+1.0))+0.1,[1,1,1,k_len])
        
        molecular_p = tf.reshape(tf.to_float(tf.range(k_len)),[1,1,1,k_len])
        positions = tf.reshape(tf.to_float(tf.range(k_len)),[1,1,k_len,1])
        molecular = tf.tile(positions, [batch_size,num_heads,1,k_len]) - tf.tile(molecular_p, [batch_size,num_heads,k_len,1])
        
        q_gaussian= 0.0 - 2*tf.pow(tf.div(molecular,5),2.0) + bias
        k_gaussian= 0.0 - 2*tf.pow(tf.div(molecular,5),2.0) + bias
       
        context_mask_q=tf.nn.softmax(q_gaussian,name="q_gaussian_weights")
        context_mask_k=tf.nn.softmax(k_gaussian,name="k_gaussian_weights")
        ...
        #q_max = tf.nn.max_pool(q*context_bias,[1,1,10,1],[1,1,1,1],'SAME')
        #context_q = tf.reshape(tf.transpose(q_max,[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        #context_q = tf.reshape(tf.transpose(tf.reduce_max(q*context_bias,2,keep_dims=True),[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        #context_q = tf.reshape(tf.transpose(tf.reduce_max(q*context_bias,2,keep_dims=True),[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        context_q = tf.reshape(tf.transpose(tf.matmul(context_mask_q,q),[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        context_matrix_q = tf.get_variable("context_matrix", [num_heads,key_depth_per_head, key_depth_per_head], dtype=dtype)
        #context_bias = tf.get_variable("context_bias", [num_heads,1, key_depth_per_head], dtype=dtype)
        context_q = tf.transpose(tf.reshape(tf.matmul(context_q,context_matrix_q),[num_heads,batch_size,-1,key_depth_per_head]),[1,0,2,3])
        
        #k_max = tf.nn.max_pool(k*context_bias,[1,1,10,1],[1,1,1,1],'SAME')
        #context_k = tf.reshape(tf.transpose(k_max,[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        #context_k = tf.reshape(tf.transpose(tf.reduce_max(k*context_bias,2,keep_dims=True),[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        context_k = tf.reshape(tf.transpose(tf.matmul(context_mask_k,k),[1,0,2,3]),[num_heads,-1,key_depth_per_head])
        context_matrix_k = tf.get_variable("context_matrix_k", [num_heads,key_depth_per_head, key_depth_per_head], dtype=dtype)
        context_k = tf.transpose(tf.reshape(tf.matmul(context_k,context_matrix_k),[num_heads,batch_size,-1,key_depth_per_head]),[1,0,2,3])
        q += context_q
        k += context_k
        ...
        # attention
        #results = multiplicative_attention(q, k, v, bias, tf.transpose(context_mask,[0,1,3,2]), keep_prob)
        results = multiplicative_attention(q, k, v, bias, keep_prob)

        #context *= key_depth_per_head ** -0.5
        #results = multiplicative_attention(context, k, v, bias, num_heads, True, keep_prob)
