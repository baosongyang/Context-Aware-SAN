        ...
        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    y = layers.attention_block.multihead_attention(
...
def encoding_graph(features, mode, params):
    ...
    src_shape=tf.shape(features["source"])
    batch_size = src_shape[0]
    src_max_len = src_shape[1]
    pad_len = 4-src_max_len%4
    src_seq = tf.concat([features["source"],tf.zeros([batch_size,pad_len],dtype=tf.int32)],axis=-1)
    src_len = features["source_length"]
    src_mask = tf.sequence_mask(src_len,
                                maxlen=src_max_len+pad_len,
                                dtype=tf.float32)
...
def decoding_graph(features, state, mode, params):
    ...
    src_max_len = tf.shape(features["source"])[1]
    pad_len = 4-src_max_len%4
