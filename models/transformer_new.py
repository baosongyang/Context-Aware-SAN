...
def transformer_encoder(inputs, bias, params, dtype=None, scope=None):
    with tf.variable_scope(scope, default_name="encoder", dtype=dtype,
        ...
        if params.context:
            ...
            #src_q = tf.zeros([tf.shape(x)[0], k_len, params.hidden_size])
            #src_k = tf.zeros([tf.shape(x)[0], k_len, params.hidden_size])

        for layer in range(params.num_encoder_layers):
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    if (layer == 0)or(layer==1)or(layer == 2):
                        localheads = params.local_heads
                    elif (layer == 3)or(layer == 4)or(layer ==5):
                        localheads = 0
                    x_pre = _layer_process(x, params.layer_preprocess)
                    if params.context:
                        global_context = tf.concat([global_context,tf.matmul(src_bias,x_pre)/src_len],axis=-1)

                    y = layers.attention_new.multihead_attention(
                        x_pre,
                        None,
                        bias,
                        params.num_heads,
